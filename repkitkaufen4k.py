...
def repkitkaufen():
    for i in range(int(choice_repkit)):
        tankeöffnen()
        warten()
        mouse.move(1318, 762, absolute=True)
        warten()
        counterLoop = 0
...
def kanisterkaufen():
    ...
    for i in range(int(choice_kanister)):
        mouse.move(1318, 842, absolute=True)
        print("klick")
        mouse.click('left')
        if stop == True:
            break
...
def kaufbestätigen():
    mouse.move(1500, 660, absolute=True)
    warten()
    mouse.click('left')
    warten()
...
def inventar():
    ...
    warten()
    # inventar 1 slot
    mouse.move(433, 333, absolute=True)
    warten()
    mouse.press('left')
    warten()
    # auto 1 slot
    mouse.move(1380, 560, absolute=1)
    warten()
    mouse.release('left')
    warten()
    keyboard.press('esc')
